...
@app.get(
    "/api/accounttypes/list",
    name="get_api_accounttypes_list",
    ...
    with app.dbconn() as conn:
        cm = api.ColumnMap(
            ...
            atype_name=api.cgen.pyhacc_accounttype.name(
                label="Type", url_key="id", sort_proxy="sort"
            ),
        )
...
where id=%(at)s"""
    ...
    results = api.Results()
    with app.dbconn() as conn:
        results.tables["accounttype", True] = api.sql_tab2(
            conn, select, {"at": atype_id}
        )
    return results.json_out()
...
def put_accounttype(atype_id):
    ...
    if len(atype.rows) != 1:
        raise api.UserError(
            "invalid-argument", "Exactly one account type must be specified."
        )
